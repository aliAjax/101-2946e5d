...
export function parseCSV(csvText: string, locationLookup: LocationLookup): CSVParseResult {
  ...
  const missingFields = CSV_REQUIRED_FIELDS.filter((f) => !headers.includes(f));

  const fieldIndex: Partial<Record<CSVField, number>> = {};
  CSV_REQUIRED_FIELDS.forEach((f) => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const transportMode = getVal('transportMode').toLowerCase();
    ...
    else if (!VALID_TRANSPORT_MODES.includes(transportMode as TransportMode))
      rowErrors.push(`交通方式"${transportMode}"无效，应为: ${VALID_TRANSPORT_MODES.join('/')}`);
